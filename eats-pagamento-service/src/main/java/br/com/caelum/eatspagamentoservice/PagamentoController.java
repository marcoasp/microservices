...
class PagamentoController {
	...
	private final PedidoRestClient pedidoClient;

	@GetMapping("/{id}")
	PagamentoDto detalha(@PathVariable("id") Long id) {
		final Pagamento pagamento = pagamentoRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException());
	...
	ResponseEntity<PagamentoDto> cria(@RequestBody Pagamento pagamento, UriComponentsBuilder uriBuilder) {
		...
		final Pagamento salvo = pagamentoRepo.save(pagamento);
		final URI path = uriBuilder.path("/pagamentos/{id}").buildAndExpand(salvo.getId()).toUri();
	...
	PagamentoDto confirma(@PathVariable("id") Long id) {
		...
		final Long pedidoId = pagamento.getPedidoId();
