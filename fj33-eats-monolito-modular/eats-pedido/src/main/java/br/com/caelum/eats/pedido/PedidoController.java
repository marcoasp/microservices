...
class PedidoController {
	...
	public void pago(@PathVariable("id") Long id) {
		final Pedido pedido = repo.porIdComItens(id);
		if (pedido == null) {
	...
	PedidoDto porId(@PathVariable("id") Long id) {
		final Pedido pedido = repo.findById(id).orElseThrow(() -> new ResourceNotFoundException());
	...
	PedidoDto adiciona(@RequestBody Pedido pedido) {
		...
		final Pedido salvo = repo.save(pedido);
